...
-- 插入角色菜单关联
-- 派单客服：菜单 4, 7, 8（老板信息管理、陪玩派单管理、陪玩报单审核）
...
-- 插入管理员账号（密码为 123123123w）
INSERT INTO `user` (`user_id`, `account`, `password`, `nickname`, `code`, `sex`, `phone`, `role`, `status`) VALUES
(147, '17796203672', '$2b$12$abcdefghijklmnopqrstuvwxyz1234567890ABCDEFG', '魔丸', 'AF209', 1, '17796203672', 6, 1);
...
| code | CHAR(10) | 邀请码，全系统唯一 |
| role | TINYINT | 角色：1=陪玩，2=客服，6=管理员 |
| player_level_id | BIGINT | 陪玩等级（关联 player_level） |
| deposit | DECIMAL(10,2) | 押金金额 |
| project_hypothecate | TINYINT | 是否已交项目担保押金：1=已担保 |
| freeze_hours | INT | 冻结小时数（陪玩被冻结时设置） |
| project_accept_time | BIGINT | 项目接单时间戳（秒级时间戳） |
...
| code | CHAR(10) | 邀请码（老板发展下线用） |
| referrer | BIGINT | 推荐人 boss_id，0=无推荐人 |
| referrer_commission | DECIMAL(10,2) | 累计推荐人佣金总额 |
| boss_type_id | BIGINT | 关联 boss_type（VIP类型） |

#### fund_flow 表（老板资金流水）

| 字段 | 类型 | 说明 |
|------|------|------|
| amount | DECIMAL(12,2) | 变动金额（充值为正，消费为负） |
| give | DECIMAL(12,2) | 赠送金额（充值时赠送） |
| balance_before / balance_after | DECIMAL(12,2) | 变动前后余额，用于账平 |
| operation_type | TINYINT | 1=消费，2=退款，3=充值，4=提现，5=系统调整 |
| extra_data | JSON | 原始请求数据（方便对账） |

#### order 表（派单）

| 字段 | 类型 | 说明 |
|------|------|------|
| no | VARCHAR(20) | 派单编号，全系统唯一 |
| category_id | JSON | 游戏项目ID数组（如 `[1, 5]` 表示选了多个项目） |
| user_id | VARCHAR(500) | 分配陪玩ID（逗号分隔字符串，如 `320,321`） |
| count | DECIMAL(10,2) | 购买数量 |
| status | TINYINT | 1=进行中，2=已完成，3=已取消 |
| feedback | VARCHAR(500) | 老板对派单的反馈 |

#### project_report 表（报单/消费记录）

| 字段 | 类型 | 说明 |
|------|------|------|
| no | VARCHAR(20) | 订单编号，全系统唯一 |
| total_amount | DECIMAL(12,2) | 订单总金额 = 项目单价 × 数量 |
| player_commission | DECIMAL(12,2) | 陪玩实际收入 |
| referrer_commission | DECIMAL(12,2) | 老板推荐人佣金 |
| shop_money | DECIMAL(12,2) | 店铺净利润 |
| project_commission | — | **由 category.commission_rate 决定，非本表字段** |
| status | TINYINT | 1=待审核，2=已通过，3=已拒绝 |
| pay_status | TINYINT | 1=待支付，2=已支付 |
| start_time / end_time | DATETIME | 项目实际开始/结束时间 |

**计算公式：**
```
total_amount = category.category_price × quantity
shop_money = total_amount - player_commission - referrer_commission
player_commission = total_amount × (category.commission_rate / 100) × (1 + player_level.percent / 100)
referrer_commission = total_amount × (boss.boss_type.percent / 100)
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| category_fid | BIGINT | 父级ID，0=顶级分类 |
| category_price | DECIMAL(10,2) | **子分类才有意义，顶级分类此字段为空** |
| commission_rate | DECIMAL(5,2) | **陪玩佣金比例（%），80=陪玩拿80%** |
| category_type | VARCHAR(20) | project=项目，gift=礼物 |

#### apply 表（提现申请）

| 字段 | 类型 | 说明 |
|------|------|------|
| check | TINYINT | 审核状态：0=待审核，1=已通过，2=已拒绝 |
| remit | TINYINT | 打款状态：0=待导出，1=已导出，2=已打款 |
...
| 表名 | 索引 | 类型 | 用途 |
|------|------|------|------|
| user | uk_account | UNIQUE | 登录查询 |
| user | idx_role | 普通 | 按角色筛选陪玩/客服 |
| user | idx_status | 普通 | 筛选正常用户 |
| boss | uk_code | UNIQUE | 邀请码查询 |
| boss | idx_referrer | 普通 | 查询某人的下级 |
| order | uk_no | UNIQUE | 派单编号查询 |
| order | idx_boss_id | 普通 | 按老板查派单 |
| order | idx_customer_id | 普通 | 按客服查派单 |
| order | idx_status | 普通 | 筛选进行中/已完成 |
| project_report | uk_no | UNIQUE | 订单编号查询 |
| project_report | idx_user_id | 普通 | 按陪玩查报单 |
| project_report | idx_status | 普通 | 筛选待审核 |
| apply | idx_user_id | 普通 | 按用户查提现 |
| apply | idx_check | 普通 | 筛选待审核提现 |
| apply | idx_remit | 普通 | 筛选待打款提现 |
| fund_flow | idx_boss_id | 普通 | 按老板查流水 |
| fund_flow | idx_created | 普通 | 按时间查流水 |
| captcha | uk_uniq_id | UNIQUE | 验证码匹配 |
| captcha | idx_expire | 普通 | 定时清理过期验证码 |

---

*数据库设计基于原站 API 响应字段逆向推导。*
